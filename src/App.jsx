...
// ─────────────────────────────────────────────
// SUPABASE DB — substitui window.storage
// ─────────────────────────────────────────────
// Toda a aplicacao usa um objeto db em memoria.
// As funcoes abaixo leem/escrevem na tabela "petshop_data"
// com uma unica linha (id=1) armazenando o JSON completo.
// Simples, sem necessidade de schema complexo.
...
async function loadDB() {
  try {
    ...
    if (error || !data) {
      // Primeira vez: insere linha padrao
      const padrao = defaultDB();
      await supabase.from("petshop_data").upsert({ id: ROW_ID, conteudo: padrao });
      return padrao;
    }
...
function defaultDB() {
  return {
    ...
    agendamentos: [],
    servicos: [
      { id: 1, nome: "Banho Pequeno", preco: "40,00", duracao: "1h", desc: "Banho completo para pets ate 10kg" },
      { id: 2, nome: "Banho Medio", preco: "55,00", duracao: "1h30", desc: "Banho completo para pets 10-20kg" },
      { id: 3, nome: "Banho Grande", preco: "70,00", duracao: "2h", desc: "Banho completo para pets acima de 20kg" },
      { id: 4, nome: "Tosa Higienica", preco: "35,00", duracao: "45min", desc: "Tosa nas regioes intimas, patinhas e orelhas" },
      { id: 5, nome: "Tosa na Tesoura", preco: "60,00", duracao: "2h", desc: "Tosa modelada na tesoura" },
      { id: 6, nome: "Banho + Tosa Completa", preco: "90,00", duracao: "2h30", desc: "Pacote completo banho e tosa" },
      { id: 7, nome: "Consulta Veterinaria", preco: "120,00", duracao: "30min", desc: "Consulta clinica geral" },
      { id: 8, nome: "Vacinacao", preco: "80,00", duracao: "15min", desc: "Aplicacao de vacinas" },
    ]
  };
}
...
function Modal({ open, onClose, title, children }) {
  ...
  return (
    <div onClick={function(e) { if (e.target === e.currentTarget) onClose(); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "24px 20px 36px", width: "100%", maxWidth: 640, maxHeight: "94vh", overflowY: "auto", animation: "slideUp .3s ease" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 18 }}>{title}</div>
          <button onClick={onClose} style={{ background: "#f1f5f9", border: "none", borderRadius: "50%", width: 34, height: 34, cursor: "pointer", fontSize: 18 }}>x</button>
        </div>
        {children}
      </div>
    </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {pet && pet.tutorTel ? (
            <Btn full onClick={function() {
              const msg = "Ola " + pet.tutorNome + "! 🐾\n\nSeu pet *" + pet.nome + "* foi cadastrado!\n\n🔑 *Codigo de acesso:* " + pet.codigo + "\n\nUse este codigo para ver o cartao de saude digital do seu pet!";
...
function ModalPet({ open, onClose, onSave, petEdit }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={petEdit ? "Editar Pet" : "Cadastrar Novo Pet"}>
      ...
      <Field label="Alergias e Restricoes"><Inp value={f.alergias} onChange={function(e) { set("alergias", e.target.value); }} placeholder="Ex: Dipirona..." /></Field>
...
function ModalAgendamento({ open, onClose, onSave, pets, servicos, agEdit }) {
  ...
  const [f, setF] = useState(vazio);
  useEffect(function() {
    if (open) setF(agEdit ? Object.assign({}, agEdit) : Object.assign({}, vazio, { petId: pets.length > 0 ? String(pets[0].id) : "" }));
  }, [open]);
  function set(k, v) { setF(function(p) { return Object.assign({}, p, { [k]: v }); }); }
    ...
    <Modal open={open} onClose={onClose} title={agEdit ? "Editar Agendamento" : "Novo Agendamento"}>
      ...
      <Field label="Servico">
        <Sel value={f.servico} onChange={function(e) {
          ...
          setF(function(p) { return Object.assign({}, p, { servico: e.target.value, preco: s ? s.preco : p.preco }); });
...
function AdminPanel({ db, onSelectPet, onLogout, onUpdateDB }) {
  ...
  function salvarAg(form) {
    ...
    if (form.id) {
      ...
      if (form.lembrete && pet && pet.tutorTel) enviarWhatsApp(pet.tutorTel, msgConfirmacao(ag, pet, db.empresa));
    }
    onUpdateDB(novo);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1e293b,#312e81)", color: "#fff", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 16px rgba(0,0,0,.2)" }}>
        <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 18 }}>{"🐾 PetShop Pro"}</div>
      ...
      <div style={{ padding: 16, maxWidth: 800, margin: "0 auto", paddingBottom: 80 }}>
          ...
          <div className="fade-in">
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 12, marginBottom: 20 }}>
              {[
                ...
                return (
                  <div key={item.label} style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: 18 }}>
            ...
            <SecTitle>{"🔔 Lembretes para Amanha"}</SecTitle>
                ...
                return (
                  <Card key={ag.id} style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                      ...
                      {ag.status === "Concluido" ? <Btn small onClick={function() { setAgEdit(ag); setModalAg(true); }} color="#64748b" outline>{"✏️ Editar"}</Btn> : null}
                      {ag.status === "Cancelado" ? (
          ...
          <div className="fade-in">
            ...
            {petsBusca.length === 0
              ...
              : petsBusca.map(function(p) {
                ...
                return (
                  <Card key={p.id} style={{ marginBottom: 10, cursor: "pointer" }} onClick={function() { onSelectPet(p.id); }}>
        ...
        {aba === "config" && (
          <div className="fade-in">
            ...
            <EmpresaForm db={db} onSave={function(emp) { onUpdateDB(Object.assign({}, db, { empresa: emp })); showToast("Dados salvos!"); }} />
          </div>
        )}
      </div>
...
function CartaoPet({ pet: petProp, db, onBack, isAdmin, onUpdatePet, onUpdateDB }) {
  ...
  useEffect(function() {
    ...
    setTimeout(function() {
      ...
      pts.forEach(function(p, i) {
        ...
        ctx.fillText(p.data ? p.data.slice(5).split("-").reverse().join("/") : "", cx(i), H - 6);
      });
    }, 80);
  }, [aba, pet.pesos]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1e293b,#312e81)", color: "#fff", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        {aba === "codigo" && isAdmin && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {pet.tutorTel ? (
                <Btn full onClick={function() {
                  const msg = "Ola " + pet.tutorNome + "! 🐾\n\nSeu pet *" + pet.nome + "* esta cadastrado!\n\n🔑 *Codigo de acesso:* " + pet.codigo + "\n\nUse este codigo para ver o cartao de saude digital do seu pet!";
                  enviarWhatsApp(pet.tutorTel, msg);
